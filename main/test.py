...
parser.add_argument(
    "--input", "-i", type=str, default="../input", help="test image folder"
)
parser.add_argument(
    "--result", "-r", type=str, default="../result", help="result folder"
)
parser.add_argument(
    "--model", "-m", type=str, default="Syn_img_lowlight_withnoise", help="model name"
)
parser.add_argument(
    "--com",
    "-c",
    type=int,
    default=1,
    help="output with/without origional image and mid result",
)
parser.add_argument(
    "--highpercent",
    "-hp",
...
parser.add_argument(
    "--maxrange", "-mr", type=int, default=8, help="linear amplification range"
)
...
print("The number of test images is " + str(len(path)) + ".")
...
opt = keras.optimizers.Adam(
    learning_rate=2 * 1e-04, beta_1=0.9, beta_2=0.999, epsilon=1e-08
)
...
maxrange = arg.maxrange / 10.0
hsvgamma = arg.gamma / 10.0
# tqdm_loader = tqdm.tqdm(total=len(path), desc="Processing", unit="image")
for i in range(len(path)):
    ...
    print("The " + str(i + 1) + "th image's name is " + img_A_path)
    ...
    gray_fake_B = (
        fake_B[:, :, 0] * 0.299 + fake_B[:, :, 1] * 0.587 + fake_B[:, :, 1] * 0.114
    )
